...
flog.info("= = = = = START muziekwweb-2-mairlist (version 2024-01-25 00:29) = = = = =", name = "bsblog")
...
# get new CD-id's
muw_ids <- ml_items_db |> filter(str_detect(title, "[A-Z][A-Z0-9]+-\\d{4}$")) |> 
...
stopifnot("- not an error, just no new Muziekweb-albums found" = nrow(muw_ids) > 0)

logmsg <- sprintf("%s new Muziekweb-albums found.", nrow(muw_ids))
flog.info(logmsg, name = "bsblog")

source("src/get_muw_metadata.R", encoding = "UTF-8")

# link to muziekweb metadata
new_track_info <- read_rds("c:/Users/nipper/Documents/BasieBeats/new_album_info.RDS")
